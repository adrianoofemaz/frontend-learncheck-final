...
const colorByScore = (v) =>
  v > 75 ? "bg-green-50" : v > 50 ? "bg-yellow-50" : "bg-red-50";
const colorByTime = (v) =>
  v < 60 ? "bg-green-50" : v < 90 ? "bg-yellow-50" : "bg-red-50"; // lebih cepat lebih hijau
const colorByAttempts = (v) =>
  v <= 1 ? "bg-green-50" : v === 2 ? "bg-yellow-50" : "bg-red-50";
        ...
        <Card className="p-6">
          <div className="flex items-center justify-between mb-4">
            ...
            <span className="text-sm text-gray-500">
              Grafik akan diganti chart saat backend siap
            </span>
          </div>
          <div className="space-y-3">
            {submoduleScores.map((s, idx) => (
              <div key={idx} className="flex items-center gap-3">
                ...
                <div className="w-14 text-right font-semibold text-gray-900">
                  {s.value}%
                </div>
              </div>
            ))}
              ...
              <tbody>
                {heatmapRows.map((row, idx) => (
                  <tr key={idx}>
                    ...
                    <td
                      className={`px-4 py-3 font-semibold text-gray-900 ${colorByScore(
                        row.score
                      )}`}
                    >
                      {row.score}%
                    ...
                    <td
                      className={`px-4 py-3 text-gray-800 ${colorByTime(
                        row.timeSec
                      )}`}
                    >
                      {row.timeSec} detik
                    ...
                    <td
                      className={`px-4 py-3 text-gray-800 ${colorByAttempts(
                        row.attempts
                      )}`}
                    >
                      {row.attempts}x
